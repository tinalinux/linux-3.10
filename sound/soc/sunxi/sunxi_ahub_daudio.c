		...
		struct sunxi_ahub_daudio_priv *sunxi_ahub_daudio, int id)
{
	...
	regmap_write(sunxi_ahub_daudio->regmap,
			SUNXI_AHUB_I2S_OUT_CHMAP0(id), 0x76543210);
	regmap_write(sunxi_ahub_daudio->regmap,
			SUNXI_AHUB_I2S_OUT_CHMAP1(id), 0xFEDCBA98);
	regmap_write(sunxi_ahub_daudio->regmap,
		...
		struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		regmap_update_bits(sunxi_ahub_daudio->regmap,
			SUNXI_AHUB_I2S_OUT_SLOT(sunxi_ahub_daudio->tdm_num),
				...
				struct snd_soc_dai *dai)
{
	struct sunxi_ahub_daudio_priv *sunxi_ahub_daudio =
					snd_soc_dai_get_drvdata(dai);
	int regbit = I2S0_RST - sunxi_ahub_daudio->tdm_num;
	...
	mutex_lock(&sunxi_ahub_daudio->mutex);
	if (sunxi_ahub_daudio->used_cnt++ == 0) {
		regmap_update_bits(sunxi_ahub_daudio->regmap,
			SUNXI_AHUB_RST, (1<<regbit), (1<<regbit));
		regmap_update_bits(sunxi_ahub_daudio->regmap,
			SUNXI_AHUB_GAT, (1<<regbit), (1<<regbit));
	}
	mutex_unlock(&sunxi_ahub_daudio->mutex);
				...
				struct snd_soc_dai *dai)
{
	...
	if (--sunxi_ahub_daudio->used_cnt == 0) {
		regmap_update_bits(sunxi_ahub_daudio->regmap,
			SUNXI_AHUB_RST, (1<<regbit), (0<<regbit));
		regmap_update_bits(sunxi_ahub_daudio->regmap,
			SUNXI_AHUB_GAT, (1<<regbit), (0<<regbit));
	}
	mutex_unlock(&sunxi_ahub_daudio->mutex);
}
...
static int sunxi_ahub_daudio_suspend(struct snd_soc_dai *dai)
{
	...
	if (sunxi_ahub_daudio->pinconfig) {
		...
		devm_pinctrl_put(sunxi_ahub_daudio->pinctrl);
	}
