		...
		if (sunxi_daudio->hdmi_en == 0) {
			...
			if (params_channels(params) - 2 > 0) {
				if (sunxi_hdmi->hdmi_format == 1)
					regmap_write(sunxi_daudio->regmap, SUNXI_DAUDIO_TX1CHMAP1, 0x23);
				else
					regmap_write(sunxi_daudio->regmap, SUNXI_DAUDIO_TX1CHMAP1, 0x32);
			}
			if (params_channels(params) - 4 > 0) {
				if (params_channels(params) == 6)
					regmap_write(sunxi_daudio->regmap, SUNXI_DAUDIO_TX2CHMAP1, 0x54);
				else
					regmap_write(sunxi_daudio->regmap, SUNXI_DAUDIO_TX2CHMAP1, 0x76);
			}
			if (params_channels(params) - 6 > 0)
				regmap_write(sunxi_daudio->regmap, SUNXI_DAUDIO_TX3CHMAP1, 0x54);
#else
