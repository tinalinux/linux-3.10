...
{"di_de",   0, 0x3, RST_GATE_SHARE, 0},
{"pcie",    0, 0x3, BUS_GATE_SHARE|RST_GATE_SHARE,                 0},
};
...
SUNXI_CLK_PERIPH(de,             DE_CFG,          24,      1,            DE_CFG,             0,         4,          0,          0,          0,          DE_CFG,          DE_GATE,         DE_GATE,       0,             31,         16,         0,              0,             &clk_lock, &com_gates[5],     0);
SUNXI_CLK_PERIPH(di,             DI_CFG,          24,      1,            DI_CFG,             0,         4,          0,          0,          0,          DI_CFG,          DI_GATE,         DI_GATE,       MBUS_GATE,     31,         16,         0,              11,            &clk_lock, &com_gates[5],     1);
...
int di_priv_enable(struct clk_hw *hw)
{
	unsigned long reg;
	struct clk_ops com_ops;
	sunxi_clk_get_periph_ops(&com_ops);
	com_ops.enable(hw);
	reg = readl(REG_ADDR(DE_GATE));
	reg = SET_BITS(16, 1, reg, 1);
	writel(reg, REG_ADDR(DE_GATE));

	reg = readl(REG_ADDR(DI_GATE));
	reg = SET_BITS(16, 1, reg, 1);
	writel(reg, REG_ADDR(DI_GATE));

	return 0;
}

void di_priv_disable(struct clk_hw *hw)
{
	unsigned long reg;
	struct clk_ops com_ops;
	...
	if (!periph->com_gate->val) {
		reg = readl(REG_ADDR(DE_GATE));
		reg = SET_BITS(16, 1, reg, 0);
		writel(reg, REG_ADDR(DE_GATE));
	}
}

void set_di_priv_ops(struct clk_ops *priv_ops)
{
	priv_ops->enable = di_priv_enable;
	priv_ops->disable = di_priv_disable;
}

struct clk_ops di_priv_ops;
...
void of_sunxi_clocks_init(struct device_node *node)
{
	...
	sunxi_set_clk_priv_ops("di", &di_priv_ops, set_di_priv_ops);
