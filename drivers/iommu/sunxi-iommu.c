 ...
 **/
static u32 *iopde_offset(u32 *iopd, unsigned int iova)
{
 ...
 * @iova:	virtual I/O address
 **/
static u32 *iopte_offset(u32 *ent, unsigned int iova)
{
	return (u32 *)__va(IOPTE_BASE(*ent)) + IOPTE_INDEX(iova);
}

/* TODO: set cache flags per prot IOMMU_CACHE */
static u32 sunxi_mk_pte(u32 page, int prot)
{
	u32 flags = 0;
...
static u32 sunxi_iommu_read(struct sunxi_iommu *iommu, u32 offset)
{
	return readl(iommu->base + offset);
}

static void sunxi_iommu_write(struct sunxi_iommu *iommu, u32 offset, u32 value)
{
...
static void sunxi_enable_device_iommu(unsigned int mastor_id, bool flag)
{
	struct sunxi_iommu *iommu = global_iommu_dev;
	u32 read_data;
	unsigned long mflag;
	...
	BUG_ON(mastor_id > 5);
	spin_lock_irqsave(&iommu->iommu_lock, mflag);
	read_data = sunxi_iommu_read(iommu, IOMMU_BYPASS_REG);
	if (flag)
		read_data &= ~(mastor_id_bitmap[mastor_id]);
	else
		read_data |= mastor_id_bitmap[mastor_id];
	sunxi_iommu_write(iommu, IOMMU_BYPASS_REG, read_data);
	spin_unlock_irqrestore(&iommu->iommu_lock, mflag);
}
...
static int sunxi_tlb_init(struct sunxi_iommu_domain *sunxi_domain)
{
	...
	/* iommu init */
	spin_lock_irqsave(&iommu->iommu_lock, mflag);
	sunxi_iommu_write(iommu, IOMMU_RESET_REG, 0x8003003f);
	sunxi_iommu_write(iommu, IOMMU_RESET_REG, 0x0);
	sunxi_iommu_write(iommu, IOMMU_RESET_REG, 0x8003003f);
	...
	sunxi_iommu_write(iommu, IOMMU_BYPASS_REG, 0x3f);
...
static irqreturn_t sunxi_iommu_irq(int irq, void *dev_id)
{
	...
	if (inter_status_reg & MICRO_TLB0_INVALID_INTER_MASK) {
		...
		pr_info("iommu int: tlb0 invalid address: 0x%x, data:0x%x\n",
			addr_reg, data_reg);
		...
		pr_info("iommu int: tlb1 invalid address: 0x%x, data:0x%x\n",
			addr_reg, data_reg);
		...
		pr_info("iommu int: tlb2 invalid address: 0x%x, data:0x%x\n",
			addr_reg, data_reg);
		...
		pr_info("iommu int: tlb3 invalid address: 0x%x, data:0x%x\n",
			addr_reg, data_reg);
		...
		pr_info("iommu int: tlb4 invalid address: 0x%x, data:0x%x\n",
			addr_reg, data_reg);
		...
		pr_info("iommu int: tlb5 invalid address: 0x%x, data:0x%x\n",
			addr_reg, data_reg);
		...
		pr_info("iommu int: L1 invalid address: 0x%x, data:0x%x, id:0x%x\n",
			addr_reg, data_reg, l1_pgint_reg);
		...
		pr_info("iommu int: L2 invalid address: 0x%x, data:0x%x, id:0x%x\n",
			addr_reg, data_reg, l2_pgint_reg);
	} else {
		pr_info("sunxi iommu int error!!!\n");
	}
...
static u32 *sunxi_alloc_iopte(u32 *sent, unsigned int iova, int prot)
{
	u32 *pent;
	u32 flags = 0;
	...
	if (sent)  {
		if (!IS_VAILD(*sent)) {
			...
			*sent = __pa(pent) | DENT_VALID;
			/* The TLB is flushed in unmap,
			 *so it needn't to flush here */
			__flush_dcache_area(sent, sizeof(*sent));
		}
		return iopte_offset(sent, iova);
	}
	return NULL;
}
...
static void sunxi_iommu_domain_destroy(struct iommu_domain *domain)
{
	...
	spin_lock(&sdm->lock);
	list_for_each_entry(mdev, &sdm->mdevs, node) {
		list_del(&mdev->node);
	}
	...
	for (i = 0; i < NUM_ENTRIES_PDE; i++) {
		...
		if (IS_VAILD(*dent)) {
			...
			__flush_dcache_area(pent, PT_SIZE);
			sunxi_tlb_invalid(iova, (u32)IOMMU_PD_MASK);
			*dent = 0;
			__flush_dcache_area(dent, sizeof(*dent));
			sunxi_ptw_cache_invalid(iova);
...
{
	struct sunxi_iommu_domain *sunxi_domain = domain->priv;
	u32 *pte_addr, *dte_addr;
	int ret = 0;

	BUG_ON(sunxi_domain->pgtable == NULL);

	size = SPAGE_ALIGN(size);
	BUG_ON(size != SZ_4K);
	mutex_lock(&sunxi_domain->dt_lock);
	dte_addr = iopde_offset(sunxi_domain->pgtable, iova);
	pte_addr = sunxi_alloc_iopte(dte_addr, iova, prot);

	if (!pte_addr) {
		pr_err("%s: Failed to map iova 0x%lx/0x%lx bytes\n",
				__func__, iova, (unsigned long)size);
		ret = -ENOMEM;
	} else {
		BUG_ON(*pte_addr & SUNXI_PTE_PAGE_VALID);
		*pte_addr = sunxi_mk_pte(paddr, prot);
		__flush_dcache_area(pte_addr, sizeof(*pte_addr));
	}
	...
	if (IOPDE_INDEX(iova_start) == IOPDE_INDEX(iova_end)) {
		...
		if (IS_VAILD(*dent)) {
			for (; iova_start <= iova_end;
				...
				*pent = 0;
				__flush_dcache_area(pent, sizeof(*pent));
...
static int sunxi_iommu_add_device(struct device *dev)
{
	...
	sunxi_enable_device_iommu(mdev->tlbid, true);
	dev_warn(iommu->dev,
			"mastor id:%d, bypass:0x%x\n",
...
static void sunxi_iommu_remove_device(struct device *dev)
{
	...
	spin_lock(&sunxi_domain->lock);
	list_for_each_entry(mdev, &sunxi_domain->mdevs, node) {
		if (mdev->dev == dev) {
		...
		struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE,
		"micro_tlb0_access_count = 0x%llx\n"
		"micro_tlb0_hit_count = 0x%llx\n"
		"micro_tlb0_hitrate = %d\n"
		"micro_tlb1_access_count = 0x%llx\n"
		"micro_tlb1_hit_count = 0x%llx\n"
		"micro_tlb1_hitrate = %d\n"
		"micro_tlb2_access_count = 0x%llx\n"
		"micro_tlb2_hit_count = 0x%llx\n"
		"micro_tlb2_hitrate = %d\n"
		"micro_tlb3_access_count = 0x%llx\n"
		"micro_tlb3_hit_count = 0x%llx\n"
		"micro_tlb3_hitrate = %d\n"
		"micro_tlb4_access_count = 0x%llu\n"
		"micro_tlb4_hit_count = 0x%llu\n"
		"micro_tlb4_hitrate = %d\n"
		"micro_tlb5_access_count = 0x%llx\n"
		"micro_tlb5_hit_count = 0x%llx\n"
		"micro_tlb5_hitrate = %d\n"
		...
		"ptwcache_hitrate = %d\n"
		"micro_tlb0_latency = 0x%llx\n"
		"micro_tlb1_latency = 0x%llx\n"
		"micro_tlb2_latency = 0x%llx\n"
		"micro_tlb3_latency = 0x%llx\n"
		"micro_tlb4_latency = 0x%llx\n"
		"micro_tlb5_latency = 0x%llx\n",
		micro_tlb0_access_count,
		micro_tlb0_hit_count,
		micro_tlb0_hitrate,
		micro_tlb1_access_count,
