...
static void fc_drm_up(hdmi_tx_dev_t *dev, fc_drm_pb_t *pb);
						...
						productParams_t *prod)
{
	...
	if (dev->snps_hdmi_ctrl.hdmi_on == 0) {
		HDMI_INFO_MSG("DVI mode selected: packets not configured\n");
		return TRUE;
	}
	...
	if (video->mHdmiVideoFormat == 0) {
		...
		HDMI_INFO_MSG("Is a 3D video format, do not configure spd\n");
	}
...
static void fc_drm_up(hdmi_tx_dev_t *dev, fc_drm_pb_t *pb)
{
	...
	if (pb != 0) {
		...
		dev_write(dev, FC_DRM_PB22, 0x00);
		dev_write(dev, FC_DRM_PB23, 0x00);
		dev_write(dev, FC_DRM_PB24, 0x00);
		dev_write(dev, FC_DRM_PB25,  0x00);
	 }
