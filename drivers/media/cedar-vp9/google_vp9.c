		...
		unsigned long arg)
{
	...
	case IOCTL_GET_IOMMU_ADDR:
	{
		...
		pVeIommuBuf->ve_dma_buf = (void *)dma_buf_get(pVeIommuBuf->fd);
		if (pVeIommuBuf->ve_dma_buf < 0) {
			google_vp9_printk(KERN_ERR,
						"vp9 get dma_buf error");
			return -EFAULT;
		}

		pVeIommuBuf->ve_dma_buf_attachment =
							   (void *)dma_buf_attach((struct dma_buf *)pVeIommuBuf->ve_dma_buf,
														google_vp9_devp->platform_dev);
		if (pVeIommuBuf->ve_dma_buf_attachment < 0) {
			google_vp9_printk(KERN_ERR,
				"vp9 get dma_buf_attachment error");
			goto RELEASE_DMA_BUF;
		}

		pVeIommuBuf->ve_sg_table = (void *)dma_buf_map_attachment(
								(struct dma_buf_attachment *)pVeIommuBuf->ve_dma_buf_attachment,
										DMA_BIDIRECTIONAL);
		if (pVeIommuBuf->ve_sg_table < 0) {
			google_vp9_printk(KERN_ERR,
				"vp9 get sg_table error\n");
			goto RELEASE_DMA_BUF;
		}

		ret = dma_map_sg(google_vp9_devp->platform_dev,
								((struct sg_table *)pVeIommuBuf->ve_sg_table)->sgl,
								((struct sg_table *)pVeIommuBuf->ve_sg_table)->nents,
								DMA_BIDIRECTIONAL);
		if (ret != 1) {
			google_vp9_printk(KERN_ERR,
		...
		pVeIommuBuf->iommu_addr = sg_dma_address(((struct sg_table *)pVeIommuBuf->ve_sg_table)->sgl);
		...
		pVeIommuBuf->ve_dma_buf,
		pVeIommuBuf->ve_dma_buf_attachment,
		pVeIommuBuf->ve_sg_table,
		((struct sg_table *)pVeIommuBuf->ve_sg_table)->nents);
			...
			if (pVeIommuBuf->ve_dma_buf_attachment > 0) {
				if (pVeIommuBuf->ve_sg_table > 0) {
					dma_unmap_sg(google_vp9_devp->platform_dev,
								...
								((struct sg_table *)pVeIommuBuf->ve_sg_table)->nents,
								DMA_BIDIRECTIONAL);
					dma_buf_unmap_attachment(pVeIommuBuf->ve_dma_buf_attachment,
										(struct sg_table *)pVeIommuBuf->ve_sg_table,
											DMA_BIDIRECTIONAL);
				}

				dma_buf_detach((struct dma_buf *)pVeIommuBuf->ve_dma_buf,
							(struct dma_buf_attachment *)pVeIommuBuf->ve_dma_buf_attachment);
			}

			dma_buf_put((struct dma_buf *)pVeIommuBuf->ve_dma_buf);
			return -1;
		}
		kfree(pVeIommuBuf);
		break;
	...
	case IOCTL_FREE_IOMMU_ADDR:
	{
		...
		aw_mem_list_for_each_entry(pVeIommuBuf, &google_vp9_devp->list, i_list)
		{
			if (pVeIommuBuf->fd == sUserIommuParam.fd) {
				google_vp9_printk(KERN_DEBUG,
				"free: fd:%d, iommu_addr:%lx, dma_buf:%p, dma_buf_attach:%p, sg_table:%p nets:%d\n",
				pVeIommuBuf->fd,
				pVeIommuBuf->iommu_addr,
				pVeIommuBuf->ve_dma_buf,
				pVeIommuBuf->ve_dma_buf_attachment,
