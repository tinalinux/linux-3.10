...
int xradio_register_bh(struct xradio_common *hw_priv)
{
	...
	if (IS_ERR(hw_priv->bh_thread)) {
		...
		int ret = sched_setscheduler(hw_priv->bh_thread, SCHED_FIFO, &param);
		if (ret)
...
tx:
		...
		if (tx && tx_allowed) {
			...
			if (ret <= 0) {
				...
				/* Check for burst. */
				if (tx_burst > 1 || (tx_bursted < BH_TX_BURST_NONTXOP &&
