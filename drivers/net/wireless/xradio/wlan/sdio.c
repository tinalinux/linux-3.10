...
static int sdio_suspend(struct device *dev)
{
	...
	if (sdio_self.core) {
		struct xradio_common *hw_priv =
			(struct xradio_common *)sdio_self.core;
		/*if umac did not call xradio_wow_suspend, here to call it.*/
		if (atomic_read(&hw_priv->suspend_state) == XRADIO_RESUME) {
			sbus_printk(XRADIO_DBG_NIY, "%s xradio_wow_suspend.\n", __func__);
			sdio_self.load_state = SDIO_SUSPEND_WOW;
			return xradio_wow_suspend(hw_priv->hw, NULL);
		}
	}
...
static int sdio_resume(struct device *dev)
{
	...
	if (sdio_self.core &&
		sdio_self.load_state == SDIO_SUSPEND_WOW) {
		/* here to call xradio_wow_resume if umac did not
		 * call xradio_wow_suspend.*/
		struct xradio_common *hw_priv =
			(struct xradio_common *)sdio_self.core;
		sbus_printk(XRADIO_DBG_NIY, "%s xradio_wow_resume.\n", __func__);
		sdio_self.load_state = SDIO_LOAD;
		xradio_wow_resume(hw_priv->hw);
	}
	return 0;
}
...
struct device *sbus_sdio_init(void *hw_priv, struct sbus_ops **sdio_ops,
