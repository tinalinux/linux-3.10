...
void wms_send_deauth_to_self(struct xradio_common *hw_priv,
							 struct xradio_vif *priv)
{
...
void wms_send_disassoc_to_self(struct xradio_common *hw_priv,
							   struct xradio_vif *priv)
{
	struct sk_buff *skb = NULL;
			...
			wsm_get_interface_for_tx(struct xradio_common *hw_priv)
{
	...
	if (!down_trylock(&hw_priv->vif_init_sync)) {
		spin_lock(&hw_priv->vif_list_lock);
