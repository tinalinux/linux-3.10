...
static int sunxi_crypt_by_emce(struct crypt_config *cc);
...
static int sunxi_crypt_by_emce(struct crypt_config *cc)
{
	/*partition type need to be GPT*/
	#if 0
		if (!strcmp(cc->dev->bdev->bd->info->volname, "UDISK"))
			return 1;

		return 0;
	#else
	/*FIXME:GPT is not ready now,just for debug*/
		return 1;
	#endif
}
#endif
...
static void crypt_endio(struct bio *clone, int error)
{
	...
	if (rw == WRITE)
#else
	if ((!sunxi_crypt_by_emce(cc)) && (rw == WRITE))
#endif
	...
	if (rw == READ && !error) {
#else
	if ((!sunxi_crypt_by_emce(cc)) && (rw == READ && !error)) {
#endif
